...
void mpu6500_read_gyro(fp32 gyro[3])
{
    ...
    temp_imu_data = ((int16_t)((buf[0]) << 8) | buf[1])&0xffd0;
    gyro[0] = temp_imu_data * GYRO_SEN;
    temp_imu_data = ((int16_t)((buf[2]) << 8) | buf[3])&0xffd0;
    gyro[1] = temp_imu_data * GYRO_SEN;
    temp_imu_data = ((int16_t)((buf[4]) << 8) | buf[5])&0xffd0;
...
void mpu6500_read(fp32 gyro[3],fp32 accel[3],fp32 *temperature)
{
	uint8_t buf[14];
  int16_t temp_imu_data = 0;
  mpu6500_read_muli_reg(MPU_ACCEL_XOUT_H, buf, 14);
	temp_imu_data = ((int16_t)((buf[0]) << 8) | buf[1])&0xffd0;
  accel[0] = (temp_imu_data * ACCEL_SEN);
  temp_imu_data = ((int16_t)((buf[2]) << 8) | buf[3])&0xffd0;
  accel[1] = (temp_imu_data * ACCEL_SEN);
  temp_imu_data = ((int16_t)((buf[4]) << 8) | buf[5])&0xffd0;
  accel[2] = (temp_imu_data * ACCEL_SEN);
  temp_imu_data = (int16_t)((buf[6]) << 8) | buf[7];
	*temperature = temp_imu_data * MPU6500_TEMPERATURE_FACTOR + MPU6500_TEMPERATURE_OFFSET;
	temp_imu_data = ((int16_t)((buf[8]) << 8) | buf[9])&0xffd0;
  gyro[0] = temp_imu_data * GYRO_SEN;
  temp_imu_data = ((int16_t)((buf[10]) << 8) | buf[11])&0xffd0;
  gyro[1] = temp_imu_data * GYRO_SEN;
  temp_imu_data = ((int16_t)((buf[12]) << 8) | buf[13])&0xffd0;
  gyro[2] = temp_imu_data * GYRO_SEN;
}